...
export async function generateChatResponse(
  ...
  if (!apiKey) {
    return {
      status: 500,
      body: { error: "GROQ_API_KEY is not configured on the server" },
    };
  }
  ...
  if (!message) {
    return {
      status: 400,
      body: { error: "A message is required" },
    };
  }
  ...
  try {
    const response = await fetch("https://api.groq.com/openai/v1/chat/completions", {
      ...
      body: JSON.stringify({
        model: normalizeGroqModel(env.GROQ_MODEL),
        messages: [
          {
            role: "system",
            content:
    ...
    return {
      status: 200,
      body: { text: text || "I'm processing that pattern..." },
    };
  } catch (error) {
